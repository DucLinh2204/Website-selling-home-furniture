...
<div class="container-fluid p-4">
    ...
    <form class="row" action="{{ route('admin.category.postCategoryUpdate', $category->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
            ...
            <div class="card rounded-0 border-0 shadow-sm mb-4">
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="number" class="form-control rounded-0" id="order" name="order" value="{{ $category->order }}" required>
                    </div>
                    <div class="mb-3">
                        <label for="hidden_show" class="form-label">Ẩn hiện</label>
                        <input type="number" class="form-control rounded-0" id="hidden_show" name="hidden_show" value="{{ $category->hidden_show }}" required>
                    </div>
                </div>
            </div>
        </div>
        <div class="col-md-4 mb-4">
            <button type="submit" class="btn btn-primary btn-lg rounded-0 mt-4 w-100">Sửa danh mục</button>
        </div>
    </form>
</div>
